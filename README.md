...
**&lt;必須評価項目&gt;**  
 `[floor_condition]`: 床の可視面積と障害物のなさ。  
   ※重要: 白い壁や天井、光の反射を「床」と誤認しないこと。家具の接地や重力方向から「床面」を正確に特定すること。  
`[trash_and_dirt]`: ゴミ、汚れ、不衛生な物体の有無（ないほど高得点）。  
`[general_clutter]`: 生活用品の散乱度合い（整頓されているほど高得点）。  

**&lt;条件付き評価項目（検出時のみ）&gt;**  
- 画像内に以下の家具が存在する場合のみ評価し、存在しない場合は計算から除外。  
 `[bed_state]`: ベッド・布団の整頓具合。  
 `[desk_state]`: 机・作業台の上の整理具合。  
 `[laundry_state]`: 衣類（カゴの中、干している服）の整頓具合。

これらの項目で最終的に0～100点で評価を行う。
